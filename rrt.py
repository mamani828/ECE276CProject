...
to only max_iter iterations."""
        ...
        for _ in range(self.max_iter):
            if np.random.rand() < 0.10: # get sample
                ...
                q_rand = np.array([np.random.uniform(lo, hi) for (lo, hi) in self.q_limits], dtype=float)
            ...
            if check_edge_collision(self.robot_id, self.obstacle_ids, nearest.joint_angles.tolist(), q_new.tolist(),discretization_step=0.01):
                continue
            new_node = Node(q_new)
            ...
            if np.linalg.norm(new_node.joint_angles - self.q_goal.joint_angles) <= self.step_size:
                if not check_edge_collision(
                    ...
                    new_node.joint_angles.tolist(), self.q_goal.joint_angles.tolist(),
                    discretization_step= 0.01
                ):
                    self.q_goal.parent = new_node
                    arm_id = self.robot_id
                    plot_link_coordinate_frames(arm_id, [0,1,2], axis_length=0.1, duration=0) # Visualize the robot coordinate frames
                    path = []
                    node = self.q_goal
    ...
    def plan2(self):
        ...
        for _ in range(self.max_iter):
            ...
            parent = nearest
            min_cost = nearest.cost + self.edge_cost(nearest.joint_angles, new_node.joint_angles)

            for node in near_nodes:
                if check_edge_collision(self.robot_id, self.obstacle_ids,node.joint_angles.tolist(), new_node.joint_angles.tolist(),discretization_step=0.01):
                    continue
                cost_through_node = node.cost + self.edge_cost(node.joint_angles, new_node.joint_angles)
            ...
            for node in near_nodes:
                if node is parent:
                    continue
                cost_through_new = new_node.cost + self.edge_cost(new_node.joint_angles, node.joint_angles)

                if cost_through_new + 1e-9 < node.cost:  #tiebreaker
                    
                    if not check_edge_collision(self.robot_id, self.obstacle_ids,new_node.joint_angles.tolist(), node.joint_angles.tolist(),discretization_step=0.01):
                        node.parent = new_node
                        node.cost = cost_through_new
                        self.update_descendant_costs(node)
            if np.linalg.norm(new_node.joint_angles - self.q_goal.joint_angles) <= self.step_size:
                if not check_edge_collision( self.robot_id, self.obstacle_ids, new_node.joint_angles.tolist(), self.q_goal.joint_angles.tolist(),discretization_step=0.01):
                    goal_cost_candidate = new_node.cost + self.edge_cost( new_node.joint_angles, self.q_goal.joint_angles)
